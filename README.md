...
| 彈簧 | 無質量軸向線性彈簧，含可調黏滯阻尼；不含線圈自身質量、彎曲、屈曲、互撞或塑性變形 |
...
**0.4.1 彈簧修正**：線圈隨端點連續轉向，修正跨越特定傾角時突然扭動的問題；補正質點模型的阻尼速度估計。組裝檢查會合計共用端點的所有彈簧／阻尼器，並考慮剛體偏心掛點，避免過硬或過高阻尼的組合通過單條檢查後產生數值能量增長。原有實驗參數和已保存試跑保留原值。

0.4.1 通過 **64 項自動化測試**與正式建置，新增靜止平衡、三維斜拉能量、解析阻尼、放手後作功、組裝穩定範圍與線圈連續性檢查。瀏覽器實測自由彈簧實驗桌 30 秒試跑與抓取放手流程。
...
**為什麼彈簧仍會擺動，或一顆砝碼的振幅又變大？**

彈簧的「軸向阻尼」只抑制伸縮，側擺也受環境空氣阻力影響。零阻尼或很小阻力的裝置會持續振動；耦合振子還會互相交換能量，單一砝碼的振幅變大不代表全系統憑空產生能量。可比較系統能量、耗散與外力作功，並一次只調整一個參數。

**為什麼增加阻尼反而顯示不能開始？**

極輕的物體、很多條共用端點的高勁度／高阻尼元件，或偏心掛點，都可能超過目前固定步長的驗證範圍。依診斷降低勁度或阻尼、增加質量，或把掛點移近中心後再試跑。這是數值模型的範圍限制。
